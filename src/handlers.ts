...
export const handlers: Record<string, ToolHandler> = {
  ...
  record_operation_attempt: async (args, client) => {
    ...
    // C3/I4 after #92: optional padded idempotency_key never certifies an
    // attempt-record replay pin (same helper as settle_operation).
    assertOptionalOperationIdempotencyKeyComplete(args?.idempotency_key);
    const raw = await wrapApi(
  ...
  resolve_operation_attempt: async (args, client) => {
    ...
    // C3/I4 after #92: optional padded idempotency_key never certifies an
    // attempt-resolve replay pin (same helper as settle_operation).
    assertOptionalOperationIdempotencyKeyComplete(args?.idempotency_key);
    const raw = await wrapApi(
