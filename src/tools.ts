...
Credits: 1. HTTP: POST /api/v2/context/change-impact`,
    inputSchema: {
      ...
      properties: {
        answer_revision_id: {
          type: 'string',
          description:
            'Non-blank unpadded answer revision for freshness. Blank/whitespace/surrounding-padded → incomplete_answer_revision_identity (never trim-launder).',
        },
        packet_id: {
          type: 'string',
          description:
            'Optional non-blank unpadded sealed packet id. Blank/whitespace/surrounding-padded → incomplete_packet_identity. When set with changed_ids, missing packets fail closed (change_impact_incomplete).',
        },
        changed_ids: {
          type: 'array',
          description:
  ...
  },
  {
    ...
    description: `Create a sealed evidence packet from authorized source bindings. The server builds and seals the packet — clients cannot supply a certified payload. Blank/whitespace or surrounding-padded claim_text → padded_claim_text (same honesty as selectPassage #314 / resolve_seeds text — never trim-launder into a certified sealed claim). Optional operator_class blank/padded → padded_operator_class (never trim-launder into a sealed class label). Blank/whitespace/surrounding-padded binding ids (source_version_id / source_unit_id / representation_id) → incomplete_binding_identity (W3 #264/#279 pad honesty — never trim-launder into a certified sealed packet). Optional binding snippet blank/padded → padded_binding_snippet; optional binding seed blank/padded → incomplete_binding_seed_identity (never trim-launder into a sealed packet). Optional idempotency_key blank/padded → incomplete_operation_idempotency_identity (never trim-launder into a sealed packet replay; same honesty as settle/link #92/#94).

Scope comes from the authenticated API. Pass a non-blank unpadded idempotency_key to settle once under retries.
    ...
    inputSchema: {
      ...
      properties: {
        claim_text: {
          type: 'string',
          description:
            'Non-blank unpadded claim text frozen into the packet assertions. Blank/whitespace/surrounding-padded → padded_claim_text (never trim-launder).',
        },
        operator_class: {
          type: 'string',
          description:
        ...
        idempotency_key: {
          type: 'string',
          description:
            'Optional non-blank unpadded idempotency key so retries settle once. Blank/whitespace/surrounding-padded → incomplete_operation_idempotency_identity (never trim-launder).',
        },
      },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        filters: {
          type: 'object',
          description:
            'Optional ClaimScope filters (metric, period, entityId, …). Blank/whitespace or surrounding-padded values → padded_resolve_filter (never stripped into a certified pin).',
        },
        index: {
          type: 'array',
          items: {
...
Credits: 1. HTTP: GET /api/v2/context/proofs/applies`,
  ...
  },
  {
    ...
    description: `Publish an immutable saved-context workflow revision. Returns the stored revision identity. Scope comes from the API key. Blank/whitespace/surrounding-padded optional idempotency_key → incomplete_operation_idempotency_identity (W3 settle/link #92/#94 honesty — never trim-launder into a certified publish-once pin).
    ...
    inputSchema: {
      ...
      properties: {
        ...
        idempotency_key: {
          type: 'string',
          description:
            'Optional. When set: non-blank unpadded publish-once key. Blank/whitespace/surrounding-padded → incomplete_operation_idempotency_identity (never trim-launder).',
        },
      },
      required: ['workflow'],
    },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        idempotency_key: {
          type: 'string',
          description:
            'Optional. When set: non-blank unpadded run-once key. Blank/whitespace/surrounding-padded → incomplete_operation_idempotency_identity (never trim-launder).',
        },
      },
