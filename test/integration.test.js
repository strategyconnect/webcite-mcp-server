...
test('Q_MCP_FAILURES: invalid arg, isError, no-match success, unknown tool, bad API shape', async (t) => {
  ...
  await t.test(
    'record_operation_attempt equal-pad idempotency_key → incomplete_operation_idempotency_identity (never replay)',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'record_operation_attempt',
  ...
  // Squash-regression guard: #89 provider pad must stay fail-closed.
  await t.test(
    'record_operation_attempt surrounding-padded provider still → incomplete_attempt_provider_identity (#89 regression)',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'record_operation_attempt',
