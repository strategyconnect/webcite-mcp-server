...
test('every tool round-trips through the real server against the API', async (t) => {
  ...
  await t.test(
    'assess_support surrounding-padded idempotency_key → incomplete_operation_idempotency_identity',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'assess_support',
  ...
  await t.test(
    'assess_support whitespace-only idempotency_key → incomplete_operation_idempotency_identity',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'assess_support',
  ...
  await t.test(
    'assess_support equal-pad idempotency_key → incomplete_operation_idempotency_identity (never assess)',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'assess_support',
  ...
  // Squash-regression guard: #77/#73 claim_hash pad refuse must stay fail-closed.
  await t.test(
    'assess_support surrounding-padded claim_hash still → incomplete_claim_hash_identity (#77 regression)',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'assess_support',
  ...
  await t.test(
    'formal_check surrounding-padded idempotency_key → incomplete_operation_idempotency_identity',
    async () => {
      const before = seen.length;
      const lean = 'example : remaining 10 4 = 7 := by decide\n';
      const res = await rpc('tools/call', {
  ...
  await t.test(
    'formal_check whitespace-only idempotency_key → incomplete_operation_idempotency_identity',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'formal_check',
  ...
  await t.test(
    'formal_check equal-pad idempotency_key → incomplete_operation_idempotency_identity (never check)',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'formal_check',
  ...
  await t.test(
    'formal_check padded idempotency still preserves Lean source trailing newline (no HTTP)',
    async () => {
      const before = seen.length;
      const lean = 'example : remaining 10 4 = 7 := by decide\n';
      const res = await rpc('tools/call', {
      ...
      assert.equal(
        seen.slice(before).find((r) => r.path === '/api/v2/context/formal/check'),
        undefined,
        'must refuse before HTTP — Lean source must not be posted or trim-laundered',
      );
    },
  );

  await t.test(
    'get_change_impact surrounding-padded idempotency_key still → incomplete_operation_idempotency_identity (#106 regression)',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
  ...
  await t.test(
    'create_claim_relation surrounding-padded idempotency_key → incomplete_operation_idempotency_identity',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'create_claim_relation',
  ...
  await t.test(
    'create_claim_relation whitespace-only idempotency_key → incomplete_operation_idempotency_identity',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'create_claim_relation',
  ...
  await t.test(
    'create_claim_relation equal-pad idempotency_key → incomplete_operation_idempotency_identity (never create)',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'create_claim_relation',
...
test('Q_MCP_FAILURES: invalid arg, isError, no-match success, unknown tool, bad API shape', async (t) => {
  ...
  await t.test(
    'create_research_run surrounding-padded objective → padded_research_objective',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'create_research_run',
      ...
      assert.equal(
        res.result.structuredContent.details?.reason,
        'padded_research_objective',
      );
      assert.equal(res.result.structuredContent.details?.field, 'objective');
      assert.match(res.result.content[0].text, /blank\/whitespace\/padded/);
      // Equal pads must not trim-launder into the same certified run purpose.
      assert.equal(
        seen.slice(before).find((r) => r.path === '/api/v2/context/research-runs'),
        undefined,
      );
  ...
  await t.test(
    'create_research_run whitespace-only objective → padded_research_objective',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'create_research_run',
  ...
  await t.test(
    'reserve_operation surrounding-padded idempotency_key → incomplete_operation_idempotency_identity',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'reserve_operation',
        arguments: {
          idempotency_key: ' ik-pad-1 ',
          kind: 'parse',
          credits: 1,
        },
      });
  ...
  await t.test(
    'reserve_operation whitespace-only idempotency_key → incomplete_operation_idempotency_identity',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'reserve_operation',
  ...
  await t.test(
    'reserve_operation surrounding-padded kind → incomplete_operation_kind_identity',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'reserve_operation',
        arguments: {
          idempotency_key: 'ik-kind-1',
          kind: ' parse ',
          credits: 1,
        },
      });
  ...
  await t.test(
    'open_operation_root pad regression still refuses padded idempotency_key',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'open_operation_root',
  ...
  await t.test(
    'get_change_impact surrounding-padded idempotency_key → incomplete_operation_idempotency_identity',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'get_change_impact',
  ...
  await t.test(
    'get_change_impact whitespace-only idempotency_key → incomplete_operation_idempotency_identity',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'get_change_impact',
  ...
  // Squash-regression guard: #57/#264 answer_revision pad refuse must stay fail-closed.
  await t.test(
    'get_change_impact surrounding-padded answer_revision_id still → incomplete_answer_revision_identity (#57 regression)',
    async () => {
      const before = seen.length;
      const res = await rpc('tools/call', {
        name: 'get_change_impact',
