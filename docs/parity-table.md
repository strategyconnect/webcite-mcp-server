...
| `query_context` | `POST /api/v2/context/query` | non-blank unpadded `text`, optional non-blank unpadded `source_texts` / `source_version_ids`, non-blank unpadded `filters`, `max_hops`, `limit`, `idempotency_key` | `status`, operator, refs, gaps, presentation | **Successful no-match:** `status: refuse` / empty refs, `isError` false. Blank/whitespace/surrounding-padded `text` → `padded_select_text` (#314; never trim-launder). API `gaps: [padded_select_text]` → `invalid_api_output` (never certified refuse). Blank/padded `source_texts` → `padded_source_text`; blank/padded `source_version_ids` → `incomplete_source_version_identity`. Blank/padded filters → `padded_lookup_filter` (#307). Other invalid args → `invalid_argument`. Malformed API → `invalid_api_output`. | 1 | `sourceVersionId` + `nodeId` (presentation numbers are display-only) |
...
| `create_evidence_packet` | `POST /api/v2/context/evidence-packets` | `claim_text` (non-blank unpadded; blank/padded → `padded_claim_text`), `bindings[]` (non-blank unpadded `source_version_id` / `source_unit_id` / `representation_id`; optional non-blank unpadded `snippet` / `seed`), optional `operator_class` (blank/padded → `padded_operator_class`), `idempotency_key` | `packet_id`, optional `content_hash` / `input_packet_id`, gaps | Empty claim/bindings → `invalid_argument`. Blank/whitespace/surrounding-padded claim_text → `padded_claim_text` (#314 honesty; never trim-launder). Blank/whitespace/surrounding-padded binding ids → `incomplete_binding_identity` (#264/#279; never trim-launder). Blank/padded optional snippet → `padded_binding_snippet`; blank/padded optional seed → `incomplete_binding_seed_identity` (never trim-launder). Blank/padded optional operator_class → `padded_operator_class` (never trim-launder; restores #85 after #86 squash). Retries must reuse `idempotency_key`. Client cannot supply a certified sealed payload. | 2 | Returned `packet_id` (+ hash when present) |
