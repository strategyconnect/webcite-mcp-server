...
| `number_inventory` | `POST /api/v2/context/numbers/inventory` | `occurrences[]` with `recognition_state`, non-blank `raw`, explicit `method` (never defaulted to `native`); optional `idempotency_key` | `counts` (read/uncertain/unreadable), `coverage`, `occurrences`, `unresolved` | Incomplete identity (blank/whitespace/padded id/fragment_id)/raw/method/interpretation/recognition or unreadable+decimal → API `number_inventory_incomplete` → `api_error`. Missing `occurrences` → `invalid_argument`. | 1 | Occurrence id (dedupe); not raw magnitude |
