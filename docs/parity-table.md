...
| `number_inventory` | `POST /api/v2/context/numbers/inventory` | `occurrences[]` with `recognition_state`; optional `idempotency_key` | `counts` (read/uncertain/unreadable), `coverage`, `occurrences`, `unresolved` | Incomplete identity/recognition or unreadable+decimal → API `number_inventory_incomplete` → `api_error`. Missing `occurrences` → `invalid_argument`. | 1 | Occurrence id (dedupe); not raw magnitude |
