...
| `get_research_run` | `GET /api/v2/context/research-runs/:runId` | run_id | run | 404; flag off → refuse; blank/padded note id or ResearchScope on surfaced notes → `incomplete_eligible_note_identity` (#297) | 0 | run id; `CONTEXT_GRAPH_RESEARCH` |
| `checkpoint_research_run` | `POST /api/v2/context/research-runs/:runId/checkpoints` | expected_revision + run (wait subject + scope.tenantId non-blank/unpadded; notes id + ResearchScope non-blank/unpadded) | run | 409 stale; blank/whitespace/padded wait subject → `incomplete_wake_subject_identity`; blank/whitespace/padded scope.tenantId → `incomplete_wake_tenant_identity`; blank/padded note/scope ids → `incomplete_eligible_note_identity` (#297); flag off → refuse | 0 | checkpoint CAS; `CONTEXT_GRAPH_RESEARCH` |
