...
| `query_context` | `POST /api/v2/context/query` | text (non-blank unpadded), source_texts/ids, filters (non-blank/unpadded) | refs + plan + presentation | `refuse` ≠ HTTP error; blank/whitespace/padded text → `padded_select_text` (#314; never equal-pad-select); API gap → `invalid_api_output`; blank/padded filters → `padded_lookup_filter` (#307) | 0 | sourceVersionId / nodeId |
...
| `resolve_fragment_uses` | `POST /api/v2/context/fragments/resolve-uses` | selector; optional non-blank unpadded `packet_id` xor `answer_revision_id` (blank/padded → `incomplete_packet_identity` / `incomplete_answer_revision_identity`; never trim-launder; W3 #264/#279); else fragments?/groups?/links?/consumers?; cursor, limit | matches + matchKind + ids; `semanticSupport:false`; echoes sealed id | refuse ≠ invent; unsupported kind; blank/padded sealed ids → `invalid_argument`; sealed id + client rows → `invalid_argument` | 0 | representationId + sealed packet/answer or authorized catalog |
...
| `create_research_run` | `POST /api/v2/context/research-runs` | objective; non-blank unpadded `snapshot_id`/`workflow_version` (+ optional unpadded deal/session/root); budget; `open_requirement_ids` non-blank/unpadded | run (+ opened_root when auto) | auth; blank/padded create identities → `incomplete_create_research_identity` (#281/#297 evidenceId); blank/padded open_requirement_ids → `incomplete_loop_requirement_identity` (#304); `CONTEXT_GRAPH_RESEARCH` off → refuse | 0 | research scope |
...
| `resolve_seeds` | `POST /api/v2/context/resolve-seeds` | text, filters? (non-blank unpadded), index? (omit → SQL catalog) | candidates + leading_resolver + index_source | auth; blank/whitespace/padded filters → `padded_resolve_filter` (#311; never equal-pad-pin / bare_term fall-through) | 0 | ClaimScope seeds |
...
| `reserve_research_budget` | `POST /api/v2/context/research-runs/:runId/reserve` | non-blank unpadded `run_id`, idempotency_key, kind, credits | operationId + replay | blank/padded `run_id` → `incomplete_research_run_identity` (never trim-launder); missing root refuses; flag off → refuse | 0 | research run; `CONTEXT_GRAPH_RESEARCH` |
...
| `record_operation_attempt` | `POST /api/v2/context/operations/:operationId/attempts` | operation_id, provider, model? | attempt | auth | 0 | evidence attempt |
