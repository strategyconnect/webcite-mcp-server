...
| `get_change_impact` | `POST /api/v2/context/change-impact` | answer_revision_id and/or changed_ids (+ links/packet_id/window); blank/whitespace ids → `incomplete_changed_ids` | freshness and/or packet_impact | integrity / `change_impact_incomplete` | 0 | answer sources / packet consumers |
...
| `number_inventory` | `POST /api/v2/context/numbers/inventory` | occurrences[] (recognition_state, non-blank raw, explicit method; never invent native) | counts + coverage + occurrences | incomplete (blank/whitespace/padded identity/raw/method/interpretation/…) → `number_inventory_incomplete` | 0 | occurrence id (not magnitude) |
...
| `checkpoint_research_run` | `POST /api/v2/context/research-runs/:runId/checkpoints` | expected_revision + run (wait subject + scope.tenantId non-blank) | run | 409 stale; blank/whitespace wait subject → `incomplete_wake_subject_identity`; blank/whitespace scope.tenantId → `incomplete_wake_tenant_identity`; flag off → refuse | 0 | checkpoint CAS; `CONTEXT_GRAPH_RESEARCH` |
...
- C3 research HTTP/MCP stays dark until `CONTEXT_GRAPH_RESEARCH=1|true` (default off); create/list/get/checkpoint/reserve refuse fail-closed. Blank/whitespace wait `subjectId`/`subjectRevisionId` never wake (incomplete_wake_subject_identity). Blank/whitespace `scope.tenantId` never wakes when wait is set (incomplete_wake_tenant_identity).
