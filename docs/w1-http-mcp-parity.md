...
| `checkpoint_research_run` | `POST /api/v2/context/research-runs/:runId/checkpoints` | expected_revision + run (wait subject + scope.tenantId non-blank/unpadded) | run | 409 stale; blank/whitespace/padded wait subject → `incomplete_wake_subject_identity`; blank/whitespace/padded scope.tenantId → `incomplete_wake_tenant_identity`; flag off → refuse | 0 | checkpoint CAS; `CONTEXT_GRAPH_RESEARCH` |
