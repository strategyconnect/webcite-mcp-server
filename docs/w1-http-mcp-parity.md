...
| `get_change_impact` | `POST /api/v2/context/change-impact` | answer_revision_id and/or changed_ids (+ links/packet_id/window); blank/whitespace/padded ids → `incomplete_changed_ids`; padded/blank link endpoints → `incomplete_dependency_graph`; `source_id === consumer_id` → `self_loop_dependency` | freshness and/or packet_impact | integrity / `change_impact_incomplete` | 0 | answer sources / packet consumers |
