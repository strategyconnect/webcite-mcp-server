...
| `number_inventory` | `POST /api/v2/context/numbers/inventory` | occurrences[] (recognition_state, non-blank raw, explicit method; never invent native) | counts + coverage + occurrences | incomplete (blank/whitespace/padded identity/raw/blank_normalized_decimal blank|padded/method/interpretation/…) → `number_inventory_incomplete` | 0 | occurrence id (not magnitude) |
