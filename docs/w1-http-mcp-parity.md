...
| `query_context` | `POST /api/v2/context/query` | text (non-blank unpadded), source_texts/ids, filters (non-blank/unpadded) | refs + plan + presentation | `refuse` ≠ HTTP error; blank/whitespace/padded text → `padded_select_text` (#314; never equal-pad-select / token-strip launder); blank/padded filters → `padded_lookup_filter` (#307) | 0 | sourceVersionId / nodeId |
