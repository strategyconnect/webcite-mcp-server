...
| `create_research_run` | `POST /api/v2/context/research-runs` | objective, snapshot, workflow, budget (+ optional root); `open_requirement_ids` non-blank/unpadded | run (+ opened_root when auto) | auth; blank/padded open_requirement_ids → `incomplete_loop_requirement_identity` (#304); `CONTEXT_GRAPH_RESEARCH` off → refuse | 0 | research scope |
